...
tf.app.flags.DEFINE_integer("src_vocab_size", 1000, "Source language vocabulary size.")
...
def train():
    ...
    src_train, tgt_train, src_dev, tgt_dev, _, _, _, _, _, _ = data_utils.prepare_data(
        FLAGS.src_vocab_size, FLAGS.tgt_vocab_size)
...
def decode():
    with tf.Session() as sess:
        ...
        src_vocab_path = os.path.join(FLAGS.data_dir,
                                     "vocab%d.en" % FLAGS.src_vocab_size)
        tgt_vocab_path = os.path.join(FLAGS.data_dir,
                                     "vocab%d.fr" % FLAGS.tgt_vocab_size)
