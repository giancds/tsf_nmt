...
def _build_multicell_rnn(num_layers_encoder, num_layers_decoder, encoder_size, decoder_size,
                         source_proj_size, target_proj_size, use_lstm=True):
    encoder_cells = []
    decoder_cells = []
    ...
    for i in xrange(num_layers_encoder):
        if i == 0:
            cell = cell_class(num_units=encoder_size, input_size=source_proj_size)
        else:
            cell = cell_class(num_units=encoder_size, input_size=encoder_size)
        encoder_cells.append(cell)

    for i in xrange(num_layers_decoder):
        if i == 0:
            cell = cell_class(num_units=decoder_size, input_size=target_proj_size)
        else:
            cell = cell_class(num_units=decoder_size, input_size=decoder_size)
        decoder_cells.append(cell)

    if num_layers_encoder == 1:
        encoder_rnncell = encoder_cells[0]
    else:
        encoder_rnncell = rnn_cell.MultiRNNCell(encoder_cells)

    if num_layers_encoder == 1:
        decoder_rnncell = decoder_cells[0]
    else:
        decoder_rnncell = rnn_cell.MultiRNNCell(encoder_cells)
    ...
    def __init__(self,
        ...
        with tf.device("/cpu:0"):
            self.src_embedding = tf.Variable(
                tf.truncated_normal(
                    [source_vocab_size, source_proj_size], stddev=0.01, dtype=dtype
                ),
                name='embedding_src', trainable=True
            )
        ...
        # create the weights and biases that will
        # be used to define the initial state of the decoder
        self.Ws = tf.Variable(
            tf.truncated_normal(
                [encoder_size, encoder_size], stddev=0.01, dtype=dtype
            ),
            name='Ws', trainable=True
        )

        self.bs = tf.Variable(
            tf.truncated_normal([encoder_size], stddev=0.01, dtype=dtype),
            name='bs', trainable=True
        )
    ...
    def encode(self, source, source_r):
        ...
        with tf.name_scope('bidirectional_encoder') as scope:
            context, decoder_initial_state = nmt.bidirectional_encoder(
                    source, source_r, self.src_embedding, self.encoder_cell,
                    self.encoder_cell_r, self.batch_size, self.Ws, self.bs,
                    dtype=self.dtype
            )
...
class Seq2SeqModel(object):
    ...
    def __init__(self,
        ...
        self.encoder_cell, self.decoder_cell = _build_multicell_rnn(
                ...
                source_proj_size, target_proj_size, use_lstm=use_lstm)
